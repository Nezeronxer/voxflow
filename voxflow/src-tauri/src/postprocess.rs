...
const FILLERS_ONE: &[&str] = &[
    "эм", "эээ", "ээ", "ээм", "мм", "ммм", "мда", "хм", "ага", "типа", "um", "uh", "umm", "uhh",
    "erm", "mhm", "mm-hmm", "hmm",
];
...
/// «короче» здесь же: как вводное («короче, идём») оно пустое, но внутри
/// фразы это обычное слово («сделай текст короче») — удалять его нельзя.
const FILLERS_CONTEXTUAL: &[&str] = &["ну", "вот", "короче"];
...
pub fn is_disposable_word(word: &str) -> bool {
    ...
    FILLERS_ONE.contains(&bare.as_str())
        ...
        || is_hesitation(&bare)
        || SELF_CORRECTION_MARKERS.iter().any(|(marker, kind)| {
            marker.len() == 1
                && marker[0] == bare
                && (*kind == CorrectionKind::Cancel || marker[0] == "ой")
        })
}
...
fn remove_fillers_line(text: &str) -> String {
    ...
    let kept: Vec<&str> = words
        ...
        .filter_map(|(i, w)| {
            ...
            // «короче» — вводное только когда само отбито запятой: «короче,
            // идём». В перечислении («вежливее, короче и понятнее») и в начале
            // обычной фразы («Короче текст не станет») это прилагательное.
            let contextual = FILLERS_CONTEXTUAL.contains(&bare.as_str())
                && (i == 0 || words[i - 1].ends_with(|c: char| ",.!?…:;".contains(c)))
                && (bare != "короче" || w.ends_with(','));
...
fn find_last_self_correction_marker(
    ...
    for i in 0..toks.len() {
        for (marker, kind) in SELF_CORRECTION_MARKERS {
            ...
            if !aggressive && !marker_is_delimited(marker, kind, toks, i, len) {
                continue;
            }
...
/// Оканчивается ли токен знаком паузы («пять,», «нет.», «стоп —»).
fn ends_with_pause(raw: &str) -> bool {
    raw.chars()
        .rev()
        .take_while(|c| !c.is_alphanumeric())
        .any(|c| ",.!?…:;—–-".contains(c))
...
/// Обычные слова, которые лишь ИНОГДА бывают маркером исправления («нет»,
/// «ой», «погоди»), обязаны стоять отдельной вставкой — с паузой (знаком
/// препинания) и до, и после. Без этого «у меня нет времени» превращалось
/// в «у времени», а «погоди меня» теряло слово. Явно корректирующие слова
/// («точнее», «вернее», «отмена», «забудь») требуют паузы слева: «синей,
/// точнее зелёной». Запятая только справа не в счёт — её ставит сама
/// грамматика («не забудь, что…», «отмена, потому что…»). Двухсловные маркеры
/// («нет, стоп») говорят сами за себя. Якорная переигровка той же позиции
/// заменяет правую паузу («в пять, нет в шесть»), но не левую: иначе
/// «хочешь ты или нет, ты пойдёшь» теряло половину фразы.
fn marker_is_delimited(
    marker: &[&str],
    kind: CorrectionKind,
    toks: &[WordTok<'_>],
    start: usize,
    ...
    let anchored = correction_anchor(kind, &toks[..start], &toks[start + len..]).is_some();
    let before = start > 0 && ends_with_pause(toks[start - 1].raw);
    let after = ends_with_pause(toks[start].raw);
    match marker[0] {
        "нет" | "ой" | "погоди" | "подожди" => before && (after || anchored),
        _ => anchored || before,
    }
}
...
mod filler_tests {
    ...
    /// Регрессия «стирает слова»: «нет» как обычное слово (отрицание), «погоди»
    /// как глагол и «забудь» внутри фразы — не команды исправления. Раньше
    /// «у меня нет времени» превращалось в «у времени».
    #[test]
    fn everyday_words_that_double_as_markers_never_cut_without_a_pause() {
        let s = st(true, true);
        for phrase in [
            "У меня нет времени",
            "Сегодня нет встречи",
        ...
        // Вводное «короче» в начале — паразит, а не слово.
        assert_eq!(
            process("короче, встречаемся в шесть", &s, &[], &[]),
            "Встречаемся в шесть"
        );
        // Ту же самую вставку с паузами по-прежнему считаем исправлением.
        assert_eq!(
            process("сделай это завтра, нет, послезавтра", &s, &[], &[]),
            "Сделай это послезавтра"
        );
    ...
    /// Запятая, которую ставит сама грамматика, — не пауза исправления:
    /// «не забудь, что…», «отмена, потому что…», «X или нет, X…». Раньше эти
    /// фразы теряли начало.
    #[test]
    fn grammatical_commas_around_markers_never_cut() {
        let s = st(true, true);
        for phrase in [
            "Только не забудь, что завтра встреча",
            "Не забудь, пожалуйста, купить хлеб",
            "Пожалуйста, не забудь, что у Маши день рождения",
            "Мне нужна отмена, потому что я заболел",
            "Опиши задачу точнее, пожалуйста",
            "Так будет вернее, наверное",
            "Нравится тебе или нет, тебе придётся это сделать",
            "Прав я или нет, я узнаю завтра",
            "Хочешь ты или нет, ты пойдёшь",
            "Будет дождь или нет, будет видно завтра",
            "Перепиши письмо: сделай его вежливее, короче и понятнее",
            "Короче текст не станет",
        ] {
            assert_eq!(
                process(phrase, &s, &[], &[]),
                phrase,
        ...
        // Настоящие исправления с паузой слева работают как раньше.
        assert_eq!(
            process("на понедельник, ой, на вторник", &s, &[], &[]),
            "На вторник"
        );
        assert_eq!(process("в пять, нет в шесть", &s, &[], &[]), "В шесть");
        assert_eq!(
            process("я вчера, короче, пошёл в магазин", &s, &[], &[]),
