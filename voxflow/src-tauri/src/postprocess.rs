...
/// («точнее», «вернее», «отмена», «забудь») требуют паузы только слева:
/// «синей, точнее зелёной». Двухсловные маркеры («нет, стоп») говорят сами
/// за себя. Якорная переигровка той же позиции («на понедельник, ой, на
/// вторник») проверяется отдельно и паузы не требует.
fn marker_is_delimited(
    ...
    if correction_anchor(kind, &toks[..start], &toks[start + len..]).is_some() {
        return true;
    }
    let before = start > 0 && ends_with_pause(toks[start - 1].raw);
    let after = ends_with_pause(toks[start].raw);
    match marker[0] {
        "нет" | "ой" | "погоди" | "подожди" => before && after,
        _ => before || after,
    }
}
