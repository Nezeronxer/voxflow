...
/// Грубая оценка числа токенов. Русский на токенизаторах Qwen/GPT — примерно
/// 2 символа на токен (кириллица дробится мельче латиницы), поэтому делим на 2:
/// завышенная оценка безопасна (шире окно, больше лимит вывода), заниженная —
/// нет (обрезка ответа = потерянный текст).
pub fn estimate_tokens(text: &str) -> u32 {
    (text.chars().count() / 2 + 16).min(u32::MAX as usize) as u32
}

/// Лимит токенов ответа от длины входа: рерайт возвращает примерно тот же текст,
/// поэтому запас ×2 + 256 покрывает пунктуацию, абзацы и разметку. `cap` —
/// верхняя граница из настроек.
pub fn output_token_budget(input_tokens: u32, cap: u32) -> u32 {
    input_tokens
        .saturating_mul(2)
        .saturating_add(256)
        .min(cap.max(256))
...
/// curl завершился по таймауту `-m` (код 28)? Нужен, чтобы отличить «модель не
/// успела» от «сети нет» и сказать это пользователю прямым текстом.
pub fn curl_timed_out(status: &std::process::ExitStatus) -> bool {
    status.code() == Some(28)
}
