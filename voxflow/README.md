...
The app checks GitHub Releases for `Nezeronxer/voxflow` and installs the
`VoxFlow-Setup-*.exe` asset after the user confirms the update. Create releases
by pushing a tag like `v1.0.3`; `.github/workflows/build-installer.yml` builds
the Inno installer and uploads it to the release.
