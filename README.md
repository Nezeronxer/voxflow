...
![version](https://img.shields.io/badge/version-2.0.3-6b7cff)
...
|---|---|---|
| macOS Apple Silicon | macOS 11+, ARM64 | `VoxFlow-macOS-2.0.3-arm64-adhoc.dmg` |
| Windows | Windows 10/11, x64 | `VoxFlow-Setup-2.0.3.exe` |
...
проверяет `codesign --verify --deep --strict`, поэтому browser quarantine больше
не превращает linker-only подпись в ложную ошибку «приложение повреждено».
Ad-hoc подпись не является Apple Developer ID: при первом запуске всё ещё может
потребоваться «Системные настройки → Конфиденциальность и безопасность → Всё
равно открыть». Для установки без этого предупреждения владелец должен добавить
Developer ID + notarization; для Windows — Authenticode certificate. Секреты и
сертификаты в репозитории не хранятся.
