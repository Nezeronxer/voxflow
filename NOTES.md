...
> Репо БЕЗ git-коммитов → worktree-изоляция субагентов невозможна, делим работу по файлам.
...
## План волн (репо без коммитов → агенты по непересекающимся файлам)

- Волна 1 (параллельно): A=gigaam.rs(new), B=vad.rs(new), C=inject.rs, D=Overlay.tsx+overlay.css(new), E=styles.css+секции (темы/полировка), F=models.rs+Models.tsx (каталог GigaAM+авто-скачивание), G=db.rs (recovery+WAL), H=installer/VoxFlow.iss.
- Волна 2 (сам, серийно): engine.rs, lib.rs, settings.rs, types.ts, commands.rs, Cargo.toml — интеграция, роутинг ru→gigaam, событие level, латентность-лог по этапам.
- Волна 3: сборка + headless-селфтесты (gigaam-selftest на dataset/*.wav, замеры этапов) + тест длинной диктовки/очереди вставки.
- Волна 4: verifier-агенты по задачам 1–5 со свежим контекстом + ISCC + установка в песочницу + финальный отчёт.

## Реализация (статус по файлам)

- **gigaam.rs** (я): фронтенд log-mel64 (честный DFT-320 с таблицами, bf16-окно/банк), 3 ort-сессии, greedy RNNT
...
- **vad.rs** (я): Silero v6, input[1,576]=64ctx+512, state[2,1,128], sr — process_chunk/has_speech.
- **inject.rs** (агент C): воркер "voxflow-inject" + FIFO mpsc, ack на job, is_busy() на счётчике PENDING,
  ретрай set_text ×3, dry-режим VOXFLOW_INJECT_DRY=1 + dry_log() для тестов.
- **db.rs** (агент G): open_at() с quick_check → rename в .corrupt-<ts> + пересоздание; WAL+busy_timeout 5000.
- **engine.rs** (я): EngineCtx+gigaam/vad резиденты; warmup грузит VAD всегда + GigaAM при engine=gigaam
...
- **Overlay** (агент D, 2 захода): Overlay.tsx+overlay.css, классы aq-*, состояния idle/rec/stream/trans/done/notice,
...
- **Темы** (агент E): [data-theme="dark"] полный сет токенов (инверс-блоки в dark — светлые), init из localStorage
...
- **models.rs/Models.tsx** (агент F): каталог "gigaam-v3" (4 файла, суммарный прогресс, докачка по размеру),
...
- **lib.rs/commands.rs** (я): команда overlay_box (низ на месте, центр по X), позиция overlay в kv "overlay_pos"
  (debounce 600мс на Moved), восстановление при старте с проверкой «на экране»; --gigaam-selftest CLI.
- **Инсталлятор** (агент H): VoxFlow.iss 0.2.0 + resources\vad; tauri.conf.json 0.2.0 + resources/vad/* (я).
...
- Грабля воркфлоу: 7/8 билдеров волны 1 упали по session limit (сброс 13:40) — gigaam/vad/engine сделал сам в
  main loop, остальных перезапустил волной 1b после сброса. Часть работы упавших агентов (db.rs, Overlay.tsx)
  УЦЕЛЕЛА на диске — проверяй файлы, не верь только отчёту воркфлоу.
...
- **Верификация (6 независимых агентов со свежим контекстом)**: задачи 1–5 = PASS/PASS_WITH_NOTES,
  адверсариальное ревью ядра: BLOCKER/MAJOR нет. Ревьюер доказал бит-точность окна и мел-банка против
...
- **Боевое подтверждение**: пользователь диктовал новой сборкой в Telegram (14:30) — total 190–375 мс
  с учётом вставки, всё вставлено. В окне Claude (категория "ai") диктовки ушли в синхронный Ollama-рефайн.
