...
- P2 ✅(код) аудио: cpal-захват, ресемпл 16к, энергетический VAD, hound WAV. Мультиагентом. Реальный микрофон — ждёт ручной проверки.
...
- Визуальная проверка отрисовки React-UI в webview (tsc проходит, рантайм-рендер не заскринен — computer-use отключился).
